...
⑫ **字数是硬指标，不是建议，但不能压过大纲覆盖。** 本章应尽量落在目标字数附近。如果你写到一半发现字数不够，不要用氛围词、形容词或重复反应补字；只能回到仍服务大纲的冲突、选择、对白和后果上补足。收束前必须确认大纲关键事件已在正文中出现。若字数已接近上限但大纲后半段还没写到，不要提前章尾——进入压缩完成模式，用一个动作、一句对白或一个结果补完剩余事件，够了就收。宁可多写少量字把大纲写完，也不要在大纲还剩一半时提前结束；但禁止为了“写透”把已完成部分继续膨胀。
